...
class MNISTDataset(Dataset):
    def __init__(self, images_path, labels_path):
        ...
        with gzip.open(labels_path, 'rb') as f:
            ...
            self.labels = (labels % 2 == 1).astype(np.float32)  # 홀수: 1, 짝수 0

        # 정규화 및 Tensor 변환
        self.images = torch.tensor(self.images, dtype=torch.float32) / 255.0  # Normalize
        self.labels = torch.tensor(self.labels, dtype=torch.float32).unsqueeze(1)    # [batch_size, 1]
...
if __name__ == "__main__":
    ...
    # 모델 학습 및 평가 실행
    train(model, train_loader, loss_fn, optimizer, binary_accuracy, epochs=10)
    evaluate(model, test_loader, loss_fn, binary_accuracy)
